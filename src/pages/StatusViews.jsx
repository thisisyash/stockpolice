        ...
        <Grid>
          {viewsData.length ? (
            viewsData.map((view, index) => (
              <Paper sx={{ padding: '10px', marginBottom: '10px', display: 'flex' }} key={index}>
                <Box sx={{ width: '35%' }}>{view.name}</Box>
                <Box sx={{ width: '30%', textAlign: 'center' }}>{view.clientCode}</Box>
                <Box sx={{ width: '30%', textAlign: 'right' }}>{view.viewedOn}</Box>
              </Paper>
            ))
          ) : (
